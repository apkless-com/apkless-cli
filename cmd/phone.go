...
var lsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(phones) == 0 {
			if !all {
				fmt.Println(dim.Render("  No active phones. Use --all to include destroyed."))
			} else {
				fmt.Println(dim.Render("  No phones. Create one with: apkless create"))
			}
			return
		}
...
func runConnect(cmd *cobra.Command, args []string) {
	...
	fmt.Fprintf(os.Stderr, "\n")
	scrcpyCmd := fmt.Sprintf("scrcpy -s %s --max-size=720 -b2M --no-audio", addr)
	box := lipgloss.NewStyle().
		Border(lipgloss.RoundedBorder()).
		BorderForeground(lipgloss.Color("10")).
		Padding(0, 2).
...
var statusCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if ctx.ADBAddr != "" {
			...
			printKV("ADB", dim.Render("not connected (run: apkless connect)"))
		}
...
func init() {
	createCmd.Flags().String("region", "beijing", "Region (e.g. beijing, ap-southeast-1, us-east-1)")
	createCmd.Flags().Int("hours", 1, "Hours to allocate (1-24)")
	createCmd.Flags().Bool("wait", true, "Wait for phone to be ready")
	lsCmd.Flags().Bool("all", false, "Include destroyed phones")

	// Legacy aliases under "phone" subcommand
	phoneCmd.AddCommand(
		&cobra.Command{Use: "create", Short: createCmd.Short, Run: createCmd.Run},
		&cobra.Command{Use: "list", Short: lsCmd.Short, Aliases: []string{"ls"}, Run: lsCmd.Run},
		&cobra.Command{Use: "show", Short: showCmd.Short, Args: cobra.ExactArgs(1), Run: showCmd.Run},
		&cobra.Command{Use: "destroy", Short: rmCmd.Short, Aliases: []string{"rm", "delete"}, Args: cobra.ExactArgs(1), Run: rmCmd.Run},
		&cobra.Command{Use: "use", Short: useCmd.Short, Args: cobra.ExactArgs(1), Run: useCmd.Run},
		&cobra.Command{Use: "connect", Short: connectCmd.Short, Args: cobra.MaximumNArgs(1), Run: connectCmd.Run},
		&cobra.Command{Use: "open", Short: openCmd.Short, Args: cobra.MaximumNArgs(1), Run: openCmd.Run},
		&cobra.Command{Use: "restart", Short: restartCmd.Short, Args: cobra.ExactArgs(1), Run: restartCmd.Run},
		&cobra.Command{Use: "status", Short: statusCmd.Short, Args: cobra.MaximumNArgs(1), Run: statusCmd.Run},
	)
}
